...
public class MusicOrganizerWindow extends JFrame {
	 ...
	 */
	public RegAlbum getSelectedAlbum() {
		DefaultMutableTreeNode node = getSelectedTreeNode();
		if(node != null) {
			return (RegAlbum) node.getUserObject();
		} else {
			return null;
		}
	}
	...
	public void onAlbumAdded(Album newAlbum){
		...
		for(Enumeration e = ((DefaultMutableTreeNode) model.getRoot()).breadthFirstEnumeration(); e.hasMoreElements();){
			...
			RegAlbum parentAlbum = newAlbum.getParent();
	 ...
	 */
	public void onAlbumRemoved(RegAlbum album){
		assert album != null;
	...
	public void onClipsUpdated(){
		...
		if(a.getAlbumName().equals("Great Sound Clips") || a.getAlbumName().equals("Flagged Sound Clips")) {
			clipTable.displaySBA((SearchBasedAlbum) a);
		} else clipTable.display(a);
	}
	
	public void onClipsUpdated(RegAlbum album){
		//Album a = (Album) getSelectedTreeNode().getUserObject();
		clipTable.display(album);
	}
	
	public MusicOrganizerButtonPanel getButtonPanel() {
		return buttonPanel;
	}
}
